...
const { requirePassword, requireAdmin } = require('../middleware/auth');
...
// 0. 手动同步
router.post('/sync', requirePassword, requireAdmin, async (req, res) => {
    try {
...
// 1. 相册密码管理
router.post('/album/password', requirePassword, requireAdmin, async (req, res) => {
    try {
...
router.post('/album/verify', async (req, res) => {
    try {
        const { dir, password } = req.body;
        if (dir === undefined) return res.status(400).json({ error: "Missing directory" });
...
// 3. 删除图片
router.delete('/images/*', requirePassword, requireAdmin, async (req, res) => {
...
// 4. 删除文件
router.delete('/files/*', requirePassword, requireAdmin, async (req, res) => {
    const relPath = decodeURIComponent(req.params[0]);
    try {
        const filePath = safeJoin(STORAGE_PATH, relPath);
        if (await fs.pathExists(filePath)) {
...
// 5. 批量移动
router.post('/batch/move', requirePassword, requireAdmin, async (req, res) => {
    try {
...
// 6. 创建目录
router.post('/directories', requirePassword, requireAdmin, async (req, res) => {
    try {
        const { name } = req.body;
...
// 7. 重命名图片
router.put('/images/*', requirePassword, requireAdmin, async (req, res) => {
    const relPath = decodeURIComponent(req.params[0]);
