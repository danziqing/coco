...
// 1. Base64 上传
router.post('/upload-base64', requirePassword, requireAdmin, express.json({ limit: '50mb' }), async (req, res) => {
    try {
...
// 1.0 URL 上传
router.post('/upload-url', requirePassword, requireAdmin, express.json({ limit: '10mb' }), async (req, res) => {
    try {
        const { url } = req.body;
        if (!url) {
...
// 1.1 上传图片 (Multer)
router.post('/upload', requirePassword, requireAdmin, upload.any(), handleMulterError, async (req, res) => {
    try {
        let dir = req.body.dir || req.query.dir || "";
        dir = dir.replace(/\\/g, "/");
...
router.post('/process-image', requirePassword, requireAdmin, upload.single("image"), async (req, res) => {
    try {
      if (!req.file) {
...
// 1.3 上传文件 (任意)
router.post('/upload-file', requirePassword, requireAdmin, uploadAny.single("file"), handleMulterError, async (req, res) => {
