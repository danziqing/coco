...
import { ConfigProvider, theme, message, Spin, Grid, Modal, Button } from "antd";
...
function App() {
  ...
  useEffect(() => {
    if (!passwordRequired || isAuthenticated || isGuest) {
      api.get("/settings")
        .then(res => {
  ...
  }, [passwordRequired, isAuthenticated, isGuest]);
  ...
  return (
    ...
    >
      ...
      {isGuest && (
        <div style={{
          ...
          <Button
            ...
            style={{ color: "#fff", marginLeft: 12, fontWeight: 600, padding: 0 }}
          >
            登录
          </Button>
        </div>
      ...
      {/* Main Content */}
      <div style={{ position: "relative", minHeight: "100vh", paddingTop: isGuest ? 36 : 0 }}>
        <Suspense
          fallback={
            <div style={{
              display: "flex",
        ...
        {isApiDocs ? (
          ...
          <>
            ...
            <ImageGallery
              ...
              isAuthenticated={!passwordRequired || isAuthenticated || isGuest}
              isGuest={isGuest}
            ...
            {/* Floating Toolbar - Show when authenticated or guest */}
            {(!passwordRequired || isAuthenticated || isGuest) && (
              <FloatingToolbar
                onThemeChange={handleThemeChange}
                currentTheme={currentTheme}
